...
while ($row = $result->fetch_assoc()) {
	...
	$userlist .= '&f'.$counter.'='.urlencode(stripslashes($row['a24']));
    $userlist .= '&g'.$counter.'='.urlencode(stripslashes($row['a25']));
	
	$userlist .= '&h'.$counter.'='.urlencode(stripslashes($row['d1']));
	$userlist .= '&i'.$counter.'='.urlencode(stripslashes($row['d2']));
	
	$userlist .= '&j'.$counter.'='.urlencode(stripslashes($row['d3']));
	$userlist .= '&k'.$counter.'='.urlencode(stripslashes($row['d4']));
